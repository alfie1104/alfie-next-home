...
interface IProps {
  ...
  slideMovingRatio?: number; //한번 이동 시 움직일 거리 (itemWidth대비 몇배인지 표기)
  viewWidthRatio?: number; //화면에 보여줄 전체 item의 폭 (itemWidth 대비 몇배인지 표기)
...
function Slide({
  ...
  let hiddenedSlideWidth = slideWidth - viewWidthRatio * itemWidth; //슬라이드 내부 컨텐츠 현재 화면에 보이는 길이를 빼고 남아있는 슬라이드의 길이를 구한다.
    ...
    if (autoMoving) {
      interval = setInterval(() => {
        if (autoMovingDirection === "forward") {
          setCurrentSpot((prev) => {
            if (
              hiddenedSlideWidth - Math.abs(prev) <
              slideMovingRatio * itemWidth
            ) {
              ...
              return prev - slideMovingRatio * itemWidth; //한번에 움직여야 하는 값 만큼 빼줌
            }
          });
        } else {
          setCurrentSpot((prev) => {
            if (Math.abs(prev) < slideMovingRatio * itemWidth) {
              //슬라이드 왼쪽으로 남은 값이 한 번에 이동하는 값보다 작으면
              autoMovingDirection = "forward";
              return 0; //0까지만 이동
            } else {
              //그 외의 경우
              return prev + slideMovingRatio * itemWidth; //현재 위치에서 한 번에 이동해야 하는 값만큼 이동
            }
          });
        }
      }, autoInterval);
  ...
  const handleClickNext = (e: React.MouseEvent<HTMLElement>) => {
    e.preventDefault();
    setCurrentSpot((prev) => {
      if (hiddenedSlideWidth - Math.abs(prev) < slideMovingRatio * itemWidth) {
        //남아있는 슬라이드의 길이에서 현재 슬라이드의 위치값을 뺀 값이 한 번에 움직여야 하는 값보다 작으면
        return prev - (hiddenedSlideWidth - Math.abs(prev)); //남은 길이만큼만 이동
      } else {
        //남아있는 슬라이드의 길이가 한 번에 움직여야 하는 값 보다 크면
  ...
  return (
    <div className="flex items-center justify-center my-5">
      <button
        ...
        className="border-solid border-r-[1rem] border-r-custom-white-veryDark hover:border-l-custom-green-light border-l-0 border-b-[0.7rem] border-b-transparent border-t-[0.7rem] border-t-transparent transition-all mr-1"
      ></button>
      <ul
        style={{
          overflow: "hidden",
          width: `${viewWidthRatio * itemWidth}px`,
      ...
      <button
        ...
        className="border-solid border-l-[1rem] border-l-custom-white-veryDark hover:border-l-custom-green-light border-r-0 border-b-[0.7rem] border-b-transparent border-t-[0.7rem] border-t-transparent transition-all ml-1"
      ></button>
    </div>
  );
}
